...
resultdf3 = pd.read_csv("collected_data/Batch_3453438_batch_results_approved.csv")

print("R2",len(resultdf2))
resultdf2 = resultdf2[resultdf2.Reject != "x"]
print("R2 approved",len(resultdf2))

print("R3",len(resultdf3))
resultdf3 = resultdf3[resultdf3.Reject != "x"]
print("R3 approved",len(resultdf3))

resultdf = pd.concat([resultdf2,resultdf1,resultdf3])

new_taboos = get_new_taboos_listofdf(resultdf)
print(len(new_taboos))
make_new_table(old_taboos,new_taboos,4)
