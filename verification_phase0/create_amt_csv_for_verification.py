...
def main():
    ...
    auxpath = "aux/1_pre-pilot/items_with_qualitycontrol.csv"

    if not os.path.exists(auxpath) or input("Create data with quality controls anew? N/y").lower().startswith('y'):
        os.makedirs('aux/1_pre-pilot', exist_ok=True)
        ...
        for i, row in tqdm(df.iterrows()):
            ...
            outer_abort = 0
            while len(newnames) < MIN_CONTROL_PERCENTAGE * len(row['names_list']) and outer_abort < 10:    # TODO improve these hideous nested while loops
        ...
        df.to_csv(auxpath, index=False)
        print("Auxiliary file written to", auxpath)
    ...
    print(" Min n_names: {} (of which fillers excl. pos/syn: {})".format(df['n_names'].min(), df['n_fillers'].min()))
    print(" Max n_names: {} (of which fillers excl. pos/syn: {})".format(df['n_names'].max(), df['n_fillers'].max()))
    print(" Mean n_names: {} (of which fillers excl. pos/syn: {})".format(df['n_names'].mean(), df['n_fillers'].mean()))
    ...
    with open('1_pre-pilot/verification_{}_amt.csv'.format(PHASE), 'w+') as outfile:
