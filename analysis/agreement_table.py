...
def make_synset_df(resdf):
    ...
    new_rows = []
    for sn in syn2resp:
        new_rows.append((sn,syn2domain[sn],syn2synset[sn],syn2resp[sn],syncount[sn]))

    syndf = pd.DataFrame(new_rows,columns=['vg_obj_name','vg_domain','synset','responses','n_images'])
    ...
    for ix,row in syndf.iterrows():
        ...
        vg_prop.append(vg_weight)
        vg_overlap.append(row['responses'][row['vg_obj_name']])
