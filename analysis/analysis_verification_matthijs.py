...
if not input("Analytic or stochastic? A/s").lower().startswith('s'):
    ...
    counted = stacked.groupby(['setting', 'names_needed_95']).count().reset_index().rename(columns={'level_0': 'count'})
    ...
    sns.lineplot(x='names_needed_95', y='cumulative %', hue='setting', data=counted, hue_order=settings)
    plt.ylim((0,100))
    plt.xlim((0,36))
    plt.show()
        ...
        for i, row in tqdm(df.iterrows(), total=len(df)):
            ...
            if entry_name_info['adequacy'] >= ADEQUACY_THRESHOLD and entry_name_info['cluster_id'] == 0 and entry_name_info['cluster_weight'] >= CANONICAL_THRESHOLD:
                if False: # old way
                    ...
                    df.at[i, 'adequate names'] = {name: row['spellchecked_min2'][name] for name in row['spellchecked_min2'] if row['verified'][name]['adequacy'] >= ADEQUACY_THRESHOLD}
                    df.at[i, 'canonical object names'] = {name: row['spellchecked_min2'][name] for name in row['spellchecked_min2'] if row['verified'][name]['cluster_id'] == 0}
                    df.at[i, 'adequate canonical names'] = {name: row['spellchecked_min2'][name] for name in row['spellchecked_min2'] if row['verified'][name]['adequacy'] >= ADEQUACY_THRESHOLD and row['verified'][name]['cluster_id'] == 0}
    ...
    if False:
        ...
        counted.replace(to_replace={'setting': {'canonical object names': 'entry-level cluster names'}}, inplace=True)
        ax = sns.lineplot(x='stability', y='cumulative %', hue='setting', data=counted, hue_order=['all names', 'entry-level cluster names'], ci="sd")
