...
Our goal is to extend such models to entry-level naming, where the task is to predict the natural name of an object that most humans would choose.  
Based on large-scale, thouroughly verified object naming data from many annotators, we conduct an extensive analysis of the ability of object detection and classification models to account for entry-level object naming.
We show that object classification models ``spontaneously'' learn good entry-level names and that they can be further finetuned via transfer learning. 
\sz{A short sentence on human-like errors?}
\cs{"ManyNames on top of VG allows us to systematically vary the vocabulary object detection/classification is trained on, according to different frequecy-based or agreement-based criteria".}
\end{abstract}
