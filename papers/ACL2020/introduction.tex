...
\cs{need to make clear the diff btw objects meaning the class and meaning an instance}
...
We seek to remedy this situation by providing data and systematic modeling insights on the  phenomenon of naming real-world objects, and in particular on \textbf{entry-level names}, the name people  prefer when calling a specific object (e.g.,~\name{bird} in Figure~\ref{fig:duck}, right image). 
...
The labels seem often to be unintuitive from a human perspective---they may be "basic-level" (e.g., \name{bus}), but also very specific labels (e.g.,\ \name{goblet, gyromitra}; cf.\ the label inventory of the ILSVRC challenges; \citealt{ILSVRC15}). 
...
Moreover, as psycholinguistic studies found, humans have a preference towards a particular name, defined as the \textit{entry-level name} (e.g.,\ \name{duck}) when naturally calling an object  \cite{rosch1976basic,Rosch1978,jolicoeur1984pictures}. 
Note, though, that this research has focused on \categories\footnote{When a sub-ordinate level \category is atypical, such as penguins not being very typical birds, this affects their entry-level name (it is common to call a sparrow, but not a penguin, \name{bird}).}, as opposed to instances \lv is encompassed with, for which humans may prefer different names for instances of the same \category, and have them even disagree in their choice for the same instance (see also \citealt{graf2016animal}). 
For example, Figure~\ref{fig:duck} shows two instances of a duck, and when people were asked to name the highlighted object, most people ($27$) called the instance on the left \name{bird}, while \name{duck} was strongly preferred for the right instance. 
Hence, the single-ground truth view of CV models %, which was generally adopted by \lv models, 
