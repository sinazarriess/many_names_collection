...
The previous section looked at the performance of the different naming models using a standard single-label evaluation scheme.
%vanilla evaluation methods like accuracy.
...
Besides \textit{Hit} (the entry-level name) and \textit{Off} (none of the following categories, representing a wrong name), we distinguish between different \textit{Human-like mishits}:\\\vspace{-2ex}

\begin{tabularx}{.46\textwidth}{@{~}lX}
	\textbf{\sameobject:} & alternative name for the same object, estimated from \mn and including WordNet synonyms and hypernyms (\name{house/building}; \citealt{fellbaum1998wordnet}). \\
%(see Section~\ref{sect:mn_verification}), or a synonym or a hypernym (\name{house/building}) \\
	...
	\textbf{Related:}& a hyponym or co-hyponym of the entry-level (\textit{jacket/shirt}; also estimated from WordNet)\\
\end{tabularx}

%Semantically related names are determined with WordNet 
%\iffalse
%\begin{itemize}
...
Table \ref{tab:exp_errors_agreement} shows a break-down of model predictions and errors for objects with high and low naming agreement in ManyNames, i.e.\ objects where more (viz.\ less) than 90\% of the annotators opted for the entry-level name. 
