...
Given the entry-level naming data from ManyNames which extends the \arbitrary naming data from \vg, we now have a variety of possibilities to train, re-train or fine-tune existing object detection or classification models, to capture natural naming.
...
A pervasive trend in these results is that all models perform better 
...
The results therefore indicate that the even the FRCNN detection models do learn entry-level naming, with an accuracy of up to~$74.5$\%.
 ...
 The overall best model is FRCNN$_\text{VG1600}$$_\text{-VGMN}$, the Vanilla classifier that uses  FRCNN$_\text{VG1600}$ features and is trained on the \mn training data. 
...
As Section \ref{sec:manynames} showed, object naming is a linguistically complex phenomenon, which involves a lot of variation when names are freely annotated, but also a lot of agreement.
Hence, it is a striking result that even models that are trained on (a lot of) \arbitrary names capture entry-level naming to a good extent.
...
The next Section elaborates on this point and proposes a more fair assessment of naming quality achieved by the different models.
