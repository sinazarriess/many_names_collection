...
\begin{itemize}
	...
	\item Problems \mw{I already had a go writing a 'bridge' paragraph containing this; see below.}: referential uncertainty: object other than target object was  identified as named
	\item \cs{Mention how agreement on \mn is computed (anticipating that we'll be taking a closer look at this in 3.3 when the verification is in place).}
\end{itemize}


\mw{Sina wrote this in 3.3; I think it should go here in 3.1., in order for 3.3 to more clearly show the added value of verification:}
...
\cs{Make formulation consistent with what is (was) in the intro:}
...
Instead, we observe that there are many pairs of entry-level names that do not have a fixed preference ordering across instances, as illustrated in Figure~\ref{fig:duck} where the rather non-prototypical instance of a \name{duck} is named \name{bird} by most annotators.
...
Regarding our second reason, feature (ii) mentioned above, Figure~\ref{fig:entry-level-name-stability} shows that gathering many annotations is necessary for reliably identifying entry-level names.
\begin{figure}[t]
