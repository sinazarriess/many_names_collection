...
Our findings in Sections~\ref{sec:analysis} and~\ref{sec:taxo} are cause to question the appropriateness of the standard evaluation procedure of object naming or  detection methods, in which models are compared against a single symbol. 
%
%Let us assume that we can define, for each instance, it's canonical name as the most frequently produced one. 
Specifically, the assumption that there exists a canonical name for individual objects (instances), cannot be made in general.
...
Further, as discussed in Section~\ref{sec:taxo}, many name alternatives are not hierarchically related to the VG name. 
Hence, to distinguish automatically whether a predicted name is a true error (e.g.,\ XX) or actually a valid alternative to the given "ground truth" (e.g.,\ XX), simple methods, such as querying a taxonomic hierarchy, do not suffice. 
...
\textbf{TBC, after results discussion done}
