...
In contrast to existing work which uses multi-label data also during training (cf.\ Section\ \ref{sec:relwork}\cs{check that this is said there}), we use a classification model that was trained on single names---VG's object names (and attributes). %, i.e., it did not see multiple names for individual objects during training.  
...
\cs{And with this argumentation, we'd ideally we compare to a model fine-tuned on our data ... so much future work}
...
\cs{diffs btw. All, VGnotMN-1 and $\neg$Training  $\leftrightarrow$ overfitting?}
\iffalse
\begin{itemize}
