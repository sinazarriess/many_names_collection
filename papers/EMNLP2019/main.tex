...
%  , with an average of 3 names elicited for each object in the dataset
%  a substantial but far from perfect level of agreement in how humans call individual instances: on average, 70\% of the names for any given image coincide. Moreover, agreement varies depending on the domain (it is very high for animals, low for people).
...
%\cs{What does "decent" mean?}
  ...
  % Moreover, referential uncertainty is a major source of variation (\textit{man-shirt} ), which poses challenges for studying name variation on a large scale.
