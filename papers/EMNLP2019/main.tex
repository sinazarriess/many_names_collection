...
This typically rests on the assumption that linguistic variants of these canonical names that would occur in natural naming situations (e.g. when referring to an object) can be retrieved %via their lexical relations in
...
%multiple names (from 36 subjects) for 25k objects of different categories annotated in VisualGenome.
...
%When linking to WordNet, 
We furthermore observe that most of the alternatives for the original name in VisualGenome cannot be retrieved via hierarchical relations encoded in WordNet. %, as more complex relations like cross-classification of objects, metonymy or variation due to visual uncertainty account for a major portion of the variation.
