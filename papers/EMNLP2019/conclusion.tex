...
%In future work, we want to use the ManyNames data to fine-tune  existing models tsuch that their behavior is more human-like, and check if models trained on the ManyNames data perform better than models trained on single labels. \gbt{@Carina, pls check if this is what you meant}
...
%, and check if models trained on the ManyNames data perform better than models trained on single labels. \gbt{@Carina, pls check if this is what you meant}
...
%This of course is not an issue when working with the kind of images that traditional studies in Psycholinguistics have used, and future work will need to address this.
