...
Like object naming studies in Cognitive Science, we collect multiple names per object (concretely, 36); like most work on language \& vision, we use natural images \sz{(showing objects in complex visual contexts, surrounded by other objects, not ImageNet-like images)} on a large scale, annotating objects in 25K images from the Visual Genome dataset.
