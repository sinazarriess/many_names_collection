...
\begin{itemize}
                 \item[R0] \textbf{Lexical choice}: \\
                 Which name did the speaker use to refer to the target object?
		\item[R1] \textbf{Lexical availability}: \\
		Which names are available for the target objects?  For instance, if we want to check whether a convertible is more often referred to with a basic-level category (\textit{car}) or a more specific one (\textit{convertible}) we need to know that a given object is a \textit{convertible}, and that \textit{convertible} is a sub-level concept of \textit{car} (and vehicle, etc.). Additionally, in \cite{graf2016animal}, the authors manually group object names used by speakers in terms of three levels (sub-level, basic-level, super-level). Thus, ideally, names would be grouped according to their taxonomic relations.
		\item[R2] \textbf{Contextual informativeness}: \\
		Which names are available for the other objects in the scene (i.e. distractors)?
		For instance, when the target is a convertible, we need to know that there are multiple convertible, cars, etc. to judge whether these concepts are ambiguous. 		
