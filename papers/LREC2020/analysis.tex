...
While some of these cases are arguably annotation errors, in many cases it is not possible to distinguish which object is being indicated by the bounding box, as in the \word{bed/sleeping bag} case in Fig.~\ref{fig:ex-high-low-agreement} (image L).
...
\label{subsec:counts}
\cs{(Unless I have overseen it:) It would be good to introduce/explain what a response set is, i.e. the set of names + counts---the number of annotators having entered each of these names---and that threshold t is on the counts in the response set, etc. (and referring to Figure~5)}
...
Figure \ref{fig:ntypes} shows the cumulative histograms for type counts, i.e.\ how many objects have at least $n$\ names, with different frequency thresholds~$t$\cs{what is t actually? e.g., 'thresholds on the number of annotators entering an individual name (?)'} to account for possible noise in the crowdsourced data.
Without any frequency thresholding~\mbox{($t$=1 \cs{maybe add something like 'i.e. the name was entered by one annotator'})}, the proportion of instances that have a single name annotated is very small, below\ $10\%$, and there is a long tail of datapoints with many names, up to 19. 
...
Previous work on object naming has assumed that variation is mostly taxonomic\cs{hierarchical, to stream-line use of terms here?} (see Section~\ref{sec:rel-work}).
...
Even factoring in the noise introduced by referential uncertainty, it is clear that a good portion of our data cannot be explained by variation in the level of specificity. \cs{I'd prefer to use the term abstraction here.}
