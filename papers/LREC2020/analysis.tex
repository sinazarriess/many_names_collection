...
Figure~\ref{fig:ex-high-low-agreement} shows some example datapoints of ManyNames.
% with higher and lower agreement
As shown in Table~\ref{tab:compare} above, ManyNames gathers many more names per object than previous dataset: 35.3 on average, compared to 1-3 \gbt{to revise if we keep the VGmn column.} 
It also contains the most variability, since objects have on average 5.7 names (compared to 1-1.9).
This makes it in principle suitable to study how much inter-subject naming variation there is, and what factors influence such variation.
However, data analysis shows that object identification is still an issue in our data: despite our care in filtering out objects that are occluded or have unclear bounding boxes (see\ Section~\ref{sec:data}), we still find many examples where annotators identified different objects for the same bounding box. They typically named an adjacent object or one supported by the target object (e.g.\ \textit{jacket-toy} in Fig.~\ref{fig:ex-high-low-agreement}) or a part of the target object.
While some of these cases are arguably annotation errors, in many cases it is not possible to distinguish which object is being indicated (\textit{pants-player} or \textit{bed-sleeping bag} in Fig.~\ref{fig:ex-high-low-agreement}). 
...
In future work, we plan to filter out cases.

\subsection{Naming variation and agreement}
\label{subsec:counts}

\gbt{I think that we will need to remove the class-based analysis, for lack of space (plus, I think that the instance-level analysis is enough to get an idea of the data). For the moment I'm not doing it because I'd like to have your ok first. If we don't, we need to give an example and also say how many classes we have. Also, maybe rename ``\vg synsets'' instead of ``class''?} \sz{yes, please remove the class-based analysis. it's probably rather confusing}
\gbt{BEGIN PART NOT EDITED}
...
When discarding these names, the %response sets for instances get more consistent such that the majority contains at most 5\ names.
...
%to a manageable set of up to 5\ names per object.
...
\gbt{END PART NOT EDITED}

To assess agreement on the object names, we check the following measures, computed with $t>1$: 
%$t=1$: % 
\begin{itemize}
...
On the instance level, the most frequent name accounts for 75\%\  and the \vg name for 63\% of the responses on average. On the class level, the agreement is much lower: The average frequency of the top name drops to\ 56\% and that of the \vg name even to\ 23\%. The $H$ measures indicate a fair amount of agreement for instances, which is a bit lower than in picture norming studies on artificial images (e.g.\ \newcite{snodgrass} report an average $H$ of 0.55). On the class-level, again, an average $H$ value of 2 points to a much more even distribution of name frequencies.
...
We next turn to the sources of variation.
%and our results suggest that a large part of the factors are instance-dependent.
...
Previous work on object naming has assumed that variation is mostly taxonomical (see Section \ref{sec:relwork}).
This parameter does not explain the variation in ManyNames. 
Table \ref{tab:rel} shows the distribution of synset relations in the ManyNames responses, in terms of their type and token frequency.
To obtain it, we exploited the synset annotation in the \vg names, and added automatic linking for the additional ManyNames names to see if they stand in a taxonomic relation to the objects' \vg  synsets.%
\footnote{For detecting hypernyms, we use the hypernym closure of the synset with a depth of 10; the other relations are straightforward. The coverage of WordNet for our name data is satisfactory (90\%\ of the name types, accounting for 97\%\ of the tokens).}
...
Even factoring in the noise introduced by referential uncertainty, it is clear that a good portion of our data cannot be explained by variation in the level of specificity.
...
\bottomrule
\end{tabular}
\caption{Lexical relations of naming variants in ManyNames to annotated \vg synset, averaged over synsets. \gbt{TO DO (IMPORTANT): change the rel-not-covered example to something that is true naming variation. I moved referential uncertainty out of the analysis of sources of variation.}}
\label{tab:rel}
\vspace{-0.5cm}
\end{table}
