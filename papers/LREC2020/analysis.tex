...
As shown in Table~\ref{tab:compare} above, ManyNames gathers many more names per object than previous dataset: 35.3 on average, compared to 1-7.
It also contains the most variability, since objects have on average 5.7 names (compared to 1-1.9).
Figure~\ref{fig:ex-high-low-agreement} shows some example datapoints of ManyNames with high and low agreement.
ManyNames is in principle suitable to study how much inter-subject naming variation there is, and what factors influence such variation.
However, data analysis shows that object identification is still an issue in our data: Despite our care in filtering out objects that are occluded or have unclear bounding boxes (see\ Section~\ref{sec:data}), we still find many examples where annotators identified different objects for the same bounding box. 
...
While some of these cases are arguably annotation errors, in many cases it is not possible to distinguish which object is being indicated by the bounding box, as in the \word{bed/sleeping bag} case of in Fig.~\ref{fig:ex-high-low-agreement} (image L).
...
Previous work on object naming has assumed that variation is mostly taxonomical (see Section~\ref{sec:rel-work}).
