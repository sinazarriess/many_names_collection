...
As the table shows, previous datasets provide between one and three annotations per object, which, we believe, is not enough to assess naming behavior for individual objects.
...
Moreover, multiple annotations ($2.84$\ on average) allow for some minimal analysis of naming preferences and variation.
...
As the discussion above showed, while some existing resources can be used to shed light on factors affecting naming choices, they do not provide enough data to assess object-specific naming behavior: To what extent do different people choose the same name for a specific object? How much variation is there? 
To answer these sorts of questions, we need naming data from many subjects for the same objects; this is what we collect in ManyNames.
