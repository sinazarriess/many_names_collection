...
%\footnote{Other resources, such as \gbt{imagenet, openimages, see the other datasets cited in the openimages dataset}, provide object categories, rather than names.}
% footnote not necessary! this is explained in the background section
...
As the table shows, previous datasets provide between one and three annotations per object, which, we believe, is not enough to assess naming behavior for individual instances.
This motivates our data collection. %, in which we collect 36 names per object.
Since these datasets can, however, be useful to examine other aspects of object naming, we summarize their characteristics below.

\subsection{\refcoco and \refcocop}

Both datasets use the \referit\cite{Kazemzadeh2014} game for collecting referring expressions (RE) for natural objects in real-world images, and are built on top of MS COCO \cite{mscoco}, 
...
We expect this dataset to be good to examine the effect of distractors on naming choices in a referential setting, because images contain distractors. \cs{what does this mean, maybe give example?, e.g., from above with dog and dalmatian or so; otherwise the issue with the following part may not be understood}
Moreover, multiple annotations ($2.84$\ on average) allow for some minimal analysis of naming variation.
However, note that not all objects of an image are annotated with REs and corresponding categories, so it may be difficult to analyze the effect of distractors with automatic processing.
While in \refcoco, the elicited names can assumed to be natural, it is unclear how the additional constraints in \refcocop impact on the naturalness of object naming.
Finally, the main drawback of this dataset is that the set of categories included is quite small ($80$~COCO categories).
To sum up, we would recommend \refcoco to study the effect of distractors on object naming in a restricted set of categories.
...
This dataset has three main differences with respect to \refcoco/+: (1) the entity mentions were obtained via an image description task (captioning), as opposed to a referential task; (2) the images and the production of entity mentions were not subject to any constraints; (3) a much wider range of categories are covered (cf.\ the number of objects and the vocabulary size in Table\ \ref{tab:compare}). %\gbt{Any idea what kind of categories are covered in \flickr?}
...
This means that the identity of objects cannot be established based on the annotation, which severely limits the usefulness of the data to analyze contextual factors for naming.\cs{The explanation of intervening factors in object naming in the background may be a bit more detailed, such as to really understand the drawbacks of the existing datasets with respect to distractors.}

\subsection{Discussion}
\cs{@Sina I changed some stuff here, maybe you prefer the old version...}
As the discussion above showed, while some existing resources can be used to shed light on factors affecting naming choices, they do not provide enough data to assess instance-specific naming behavior: To what extent do different people choose the same name for an object \cs{instance?}? How much variation is there, for specific instances? 
To answer these sorts of questions, we need naming data from many subjects for the same instances. 
And, what factors affect observed naming variations (e.g.,\ the category or salience of other objects)? How does that impact the modeling of natural language in L\&V? 
Additional, dense annotations of images would be beneficial for these kinds of analyses.  
These are the motivations for our dataset, ManyNames, and for building it on top of \vg.
