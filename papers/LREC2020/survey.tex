...
In the following, we will look at their characteristics in more detail and work out requirements for a dataset that is suitable below for a large-scale, in-depth study targeted at object naming.
...
The statistics in Table \ref{tab:compare} suggest that the multiple annotations ($2.84$\ on average) actually contain a considerable amount of variation in naming (almost 2 different names on average per object). However, the small number of annotations per object would not allow for a reliable assessment of speaker agreement. 
...
A controlled analysis of the effect of context on choice in object naming, as for instance in \cite{graf2016animal}, would require substantial further data annotation as not all objects of an image are annotated with REs and corresponding categories. Hence, so-called distractor objects  \cite{krahmer:2012} and their names cannot be analyzed systematically.
Also, while in \refcoco the elicited names can assumed to be natural, it is unclear how the additional constraints in \refcocop impact on the naturalness of object naming.
Finally, the main drawback of this dataset is that the set of categories included is quite small ($80$~COCO categories).
To sum up, \refcoco is suitable for generally modeling referring expressions in context for a restricted set of categories, but less appropriate for analyzing object naming in particular.
...
The relatively low number of annotations per objects in VisualGenome (1.7 on average) shown in Table \ref{tab:compare} and the very small number of objects that have more than one name associated with it (5\%) seem to be an effect of this problem.
Thus, even though there is a different name (\textit{vulture}) for \textit{bird} object in Figure \ref{fig:bird}, this cannot be established based on the given data.
...
Table \ref{tab:compare} also shows the statistics for the subset of objects that we selected for ManyNames and, here, we find a considerably higher average of annotations per object (7). However, interestingly, the portion of objects that have different names associated with them is still extremely small.
Note that, even though RefCOCO has much less annotations per object, there are many objects with different names (70\%).
...
As the discussion above showed, while some existing resources can be used to shed light on factors affecting naming choices, they do not provide enough data to systematically assess how variable object naming really is. The RefCOCO data suggest that many objects  object-specific naming behavior: To what extent do different people choose the same name for a specific object? How much variation is there? 
To answer these sorts of questions, we need naming data from many subjects for the same objects. 
