...
However, because these datasets can be useful to examine other aspects of object naming, we summarize their characteristics below.
...
However, note that not all objects are annotated with REs and corresponding categories, so it may be difficult to analyze the effect of distractors with automatic processing.
...
\vgenome \cite{krishna2016visualgenome} is one of the most densely and richly annotated resources currently available in L\&V; here, we focus on aspects immediately relevant to object naming.
...
This means that the identity of objects cannot be established based on the annotation, which severely limits the usefulness of the data to analyze contextual factors for naming.

\subsection{Discussion}

As the discussion above showed, while some existing resources can be used to shed light on factors affecting naming choices, they do not provide enough data to assess instance-specific naming behavior: To what extent do different people choose the same name for an object? How much variation is there, for specific instances? What factors affect this variation? How does that impact modeling in L\&V?
