...
% WAS:	\caption{Cumulative histograms of the number of images and names per domain in ManyNames.}
% But it's not really a cumulative histogram
	\caption{Number of images and top responses (most frequent name per image), per domain. All high-frequency names are shown, as well as a few lower-frequency names.}
...
They are all based on McRae et al.'s~\shortcite{mcrae2005semantic} feature norms, a dataset widely used in Psycholinguistics that comprises common objects of different categories, except for \domain{people}, which we added because it is a very frequent category in \vg and a very prominent category for humans.
...
For example, \vg instances subsumed by McRae's \word{dog} were named \word{dog, beagle, greyhound, bulldog}, etc., while McRae's \word{duck}, \word{goose}, or \word{gull} did not have name variants in \vg, so we kept \word{dog} and \word{bird} (which subsumes \word{duck}, \word{goose}, or \word{gull}) as collection synsets.
...
We did not consider objects with names in plural form, with parts-of-speech other than nouns\footnote{We obtained tags with CoreNLP \cite{manning2014stanford}.}, or that were multi-word expressions (e.g.,~\word{pink bird}). 
...
\begin{figure*}[htp]
  \centering
  \includegraphics[width=1\columnwidth]{figures/round1+_p1.png}
