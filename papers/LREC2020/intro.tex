...
Picture naming norms are typically small (500-1000 images) and use idealized drawings (Figure~\ref{fig:cake}, right; \cs{but see, e.g.,~\cite{brodeur2014bank} for exceptions}); ManyNames is much larger and uses real-world images of objects in complex visual contexts, which makes it suitable for research in L\&V.
...
However, in the same way that it has proven useful to model referring expressions in visual scenes in an isolated fashion \cite{Kazemzadeh2014,mao15,Yu2016}, \cs{test models on reasoning abilities over whole scene; we observe instance-based variation, triggered by concrete appearance of object and its context, hence, analogous reasoning is required for naming, too} we believe there is value in modeling object names on their own.
