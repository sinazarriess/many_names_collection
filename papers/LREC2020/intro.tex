...
One of the central components of referring expressions are \textit{object names}; for instance, speakers may name the left object in Figure \ref{fig:cake} `cake', `food', or `dessert', a.o.
...
We start by surveying existing resources in L\&V that provide object names,
assessing their potential to contribute to the study of object naming.
While these resources can be exploited for this area of inquiry to some extent (as detailed in Section~\ref{sec:survey}), we argue that the low number of annotations per item prevents reliable assessment of \textbf{naming preferences}, on the one hand, and \textbf{variation}, on the other. 
...
Picture naming norms are typically small (500-1000 images) and use idealized drawings (Figure \ref{fig:cake}, right); ManyNames is much larger and uses real images of objects in complex visual contexts, which makes it suitable for research in L\&V.
...
However, in the same way that it has proven useful to model referring expressions in visual scenes in an isolated fashion \cite{Kazemzadeh2014,mao15,Yu2016}, we believe there is value in modeling object names on their own.
Questions that need addressing regarding object naming are 1)~how much variation in naming is attested and what factors drive the choice of a name (object category? individual properties of the object? context?); see e.g.\ Rohde et al.~\shortcite{rohde2012communicating} and Graf et al.~\shortcite{graf2016animal}, 2)~how to make L\&V models more human-like with respect to naming, improving the design of L\&V architectures (e.g.,~Lazaridou et al.~\shortcite{lazaridou-dinu-baroni:2015:ACL-IJCNLP}; Ordonez et al.,~\shortcite{Ordonez:2016}; Zhao et al.,~\shortcite{zhao2017open}).
