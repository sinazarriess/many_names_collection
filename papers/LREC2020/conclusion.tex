...
For Computer Vision and L\&V, our data highlights the fact that bounding boxes are often ambiguous \sz{does the data presented here really do this?}, which can affect model performance on object recognition and naming.
Crucially, evaluations in these tasks assume that object identification is possible based on the box; beyond showing that this is not always the case, our data can be used to assess whether model mistakes are plausible (similar to those of humans, as in the \word{toy/book/bed} case), or really off.\sz{well, this paper does not contain the verification data right=}
...
%With respect to naming variation, our
